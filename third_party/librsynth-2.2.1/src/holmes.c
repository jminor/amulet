#include <config.h>
/* $Id: holmes.c,v 1.1.1.1 2002/05/15 00:36:40 moocow Exp $
 */
char *holmes_id = "$Id: holmes.c,v 1.1.1.1 2002/05/15 00:36:40 moocow Exp $";
#include <stdio.h>
#include <ctype.h>
#include <useconfig.h>
#include <math.h>
...
/* typedefs now in rstruct.h:
     holmes_slope_t,
     holmes_trans_t,
     holmes_filter_t, holmes_filter_t_pt
*/

static float filter PROTO((holmes_filter_ptr p, Float v));

static void jsru_save PROTO((rsynth_synth *synth, double f0, float *tp));
...
static void set_trans PROTO((rsynth_synth *synth, holmes_slope_t * t, Elm_ptr a, Elm_ptr b, int ext, int e));

static void
set_trans(synth, t, a, b, ext, e)
rsynth_synth *synth;
holmes_slope_t *t;
Elm_ptr a;
Elm_ptr b;
...
int e;
{
 ...
 for (i = 0; i < nEparm; i++)
  {
   t[i].t = ((ext) ? a->p[i].ed : a->p[i].id) * synth->speed;
   if (t[i].t)
...
unsigned
holmes(synth, nelm, elm, nsamp, samp_base)
rsynth_synth *synth;
unsigned nelm;
unsigned char *elm;
unsigned nsamp;
short *samp_base;
{
 ...
 float top = 1.1 * synth->def_pars.F0hz10;
 int j;
 pars = synth->def_pars;
 ...
 pars.F4hz = 3500;
#endif
 pars.B4phz = synth->def_pars.B4phz;

 /* flag new utterance */
 parwave_init(&(synth->klatt_global));
 ...
 for (j = 0; j < nEparm; j++)
  {
   flt[j].v = le->p[j].stdy;
   flt[j].a = synth->frac;
   flt[j].b = (float) 1.0 - (float) synth->frac;
  }
 while (i < nelm)
  {
   ...
   if (dur > 0)
    {
     ...
     holmes_slope_t start[nEparm];
     holmes_slope_t end[nEparm];
     unsigned t;

     if (ce->rk > le->rk)
      {
       if (synth->par_file)
        fprintf(synth->par_file, "# %s < %s\n", le->name, ce->name);
       set_trans(synth, start, ce, le, 0, 's');
       /* we dominate last */
      }
     else
      {
       if (synth->par_file)
        fprintf(synth->par_file, "# %s >= %s\n", le->name, ce->name);
       set_trans(synth, start, le, ce, 1, 's');
       /* last dominates us */
      }
     ...
     if (synth->par_file)
      {
       int j;
       fprintf(synth->par_file, "# %s\n", ce->name);
       for (j = 0; j < nEparm; j++)
        fprintf(synth->par_file, "%c%6s", (j) ? ' ' : '#', Ep_name[j]);
       fprintf(synth->par_file, "\n");
       for (j = 0; j < nEparm; j++)
        fprintf(synth->par_file, "%c%6.4g", (j) ? ' ' : '#', start[j].v);
       fprintf(synth->par_file, "\n");
       for (j = 0; j < nEparm; j++)
        fprintf(synth->par_file, "%c%6d", (j) ? ' ' : '#', start[j].t);
       fprintf(synth->par_file, "\n");
      }
 ...
 */
       ...
       if (synth->par_file)
        {
         for (j = 0; j < nEparm; j++)
          fprintf(synth->par_file, " %6.4g", tp[j]);
         fprintf(synth->par_file, "\n");
        }
       if (synth->jsru_file)
        jsru_save(synth, pars.F0hz10 * 0.1, tp);
     ...
     if (synth->par_file)
      {
       int j;
       for (j = 0; j < nEparm; j++)
        fprintf(synth->par_file, "%c%6.4g", (j) ? ' ' : '#', end[j].v);
       fprintf(synth->par_file, "\n");
       for (j = 0; j < nEparm; j++)
        fprintf(synth->par_file, "%c%6d", (j) ? ' ' : '#', end[j].t);
       fprintf(synth->par_file, "\n");
      }
    }
   le = ce;
...
char *argv[];
{
 argc = getargs(&synth->help_only,
		"Holmes",argc, argv,
                "p", "", &(synth->par_name),  "Parameter file for plot",
                "j", "", &(synth->jsru_name), "Data for alternate synth (JSRU)",
                "S", "%d", &(synth->speed),   "Speed (1.0 is 'normal')",
                "K", "%lg", &(synth->frac),   "Parameter filter 'fraction'",
                NULL);
 if (synth->help_only)
  return argc;
...
rsynth_synth *synth;
{
  if (synth->par_name)
    {
      synth->par_file = fopen(synth->par_name, "w");
      if (!synth->par_file) {
	perror(synth->par_name);
	return 0;
      }
    }
  ...
  if (synth->jsru_name)
    {
      synth->jsru_file = fopen(synth->jsru_name, "w");
      if (!synth->jsru_file) {
	perror(synth->jsru_name);
	return 0;
      }
    }
...
{
  if (synth->par_file) {
    fclose(synth->par_file);
    synth->par_file = NULL;
  }
  if (synth->jsru_file) {
    fclose(synth->jsru_file);
...
float *tp;
{
 ...
 fputc(jsru_freq(tp[fn], 95.0, 5.0), synth->jsru_file);
 fputc(jsru_amp(tp[an]), synth->jsru_file);
 fputc(jsru_freq(tp[f1], 125.0, 25.0), synth->jsru_file);
 fputc(jsru_amp(tp[a1]), synth->jsru_file);
 fputc(jsru_freq(tp[f2], 550.0, 50.0), synth->jsru_file);
 fputc(jsru_amp(tp[a2]), synth->jsru_file);
 fputc(jsru_freq(tp[f3], 1350.0, 50.0), synth->jsru_file);
 fputc(jsru_amp(tp[a3]), synth->jsru_file);
 fputc(jsru_amp(tp[a4]), synth->jsru_file);
 fputc(jsru_amp(tp[av]), synth->jsru_file);
 fputc((int) f0, synth->jsru_file);
 fputc((int) 32, synth->jsru_file);
}
