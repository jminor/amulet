...
/* $Id: phtoelm.c,v 1.1.1.1 2002/05/15 00:36:40 moocow Exp $
 */
char *phtoelm_id = "$Id: phtoelm.c,v 1.1.1.1 2002/05/15 00:36:40 moocow Exp $";
#include <stdio.h>
#include <ctype.h>
#if defined (__STDC__)
#include <stdarg.h>
...
#if 0
#define StressDur(e,s,syn) ((e->ud + (e->du - e->ud) * s / 3)*syn->speed)
#else
#define StressDur(e,s,syn) (s,((e->du + e->ud)/2)*syn->speed)
#endif

unsigned
phone_to_elm(synth, phone, n, elm)
rsynth_synth *synth;
char *phone;
int n;
darray_ptr elm;
     ...
     while (n-- > 0)
      {
       ...
       if (!(p->feat & vwl))
        stress = 0;
       t += phone_append(elm,StressDur(p,stress,synth));
