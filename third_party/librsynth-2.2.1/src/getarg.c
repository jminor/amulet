#include <config.h>
/* $Id: getarg.c,v 1.1.1.1 2002/05/15 00:36:40 moocow Exp $
 */
char *getarg_id = "$Id: getarg.c,v 1.1.1.1 2002/05/15 00:36:40 moocow Exp $";
#if defined(__STDC__)
#include <stdarg.h>
#else
#include <varargs.h>
...
/*
   ...
   argc = getargs(synth, "Module",argc,argv,
   "r", "%d", &rate,               "Integer",
   "u", "",   &ufile_name,         "String",
   "g", "%d", &gain,               "Double",
...
#if defined(__STDC__)
int
getargs(int *help_only,char *module,int argc, char *argv[],...)
#else
int
getargs(help_only, module, argc, argv, va_alist)
int *help_only;
char *module;
int argc;
char *argv[];
